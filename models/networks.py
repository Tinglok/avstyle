...
def define_G(input_nc, output_nc, ngf, netG, norm='batch', use_dropout=False, init_type='normal',
    ...
    elif netG == 'resnet_9blocks_no_audio':
        net = ResnetGeneratorNoAudio(input_nc, output_nc, ngf, norm_layer=norm_layer, use_dropout=use_dropout, no_antialias=no_antialias, no_antialias_up=no_antialias_up, n_blocks=9, opt=opt)        
...
def define_D(input_nc, ndf, netD, n_layers_D=3, norm='batch', init_type='normal', init_gain=0.02, no_antialias=False, gpu_ids=[], opt=None):
    ...
    elif netD == 'n_layers_3d':
        net = NLayerDiscriminator3D(input_nc, ndf, n_layers=3, norm_layer=norm_layer, no_antialias=True,)
...
class ResnetGenerator(nn.Module):
    ...
    def __init__(self, input_nc, output_nc, ngf=64, norm_layer=nn.BatchNorm2d, use_dropout=False, n_blocks=6, padding_type='reflect', no_antialias=False, no_antialias_up=False, opt=None):
        ...
        model_up += [nn.Conv2d(ngf * 2, output_nc, kernel_size=7, padding=0)]
        model_up += [nn.Tanh()]

        self.model_down = nn.Sequential(*model_down)
        self.model_up = nn.Sequential(*model_up)

    def forward(self, input, embedding=None, layers=[], encode_only=False):
        if -1 in layers:
            layers.append(len(self.model_down))
        if len(layers) > 0:
            feat = input
            feats = []
            for layer_id, layer in enumerate(self.model_down):
                # print(layer_id, layer)
                feat = layer(feat)
            ...
            """Standard forward"""
            bottleneck = self.model_down(input)
            bottleneck = torch.cat((bottleneck, embedding), dim=1)
            fake = self.model_up(bottleneck)
            return fake

class ResnetGeneratorNoAudio(nn.Module):
    """Resnet-based generator that consists of Resnet blocks between a few downsampling/upsampling operations.
    We adapt Torch code and idea from Justin Johnson's neural style transfer project(https://github.com/jcjohnson/fast-neural-style)
    """
    ...
    def __init__(self, input_nc, output_nc, ngf=64, norm_layer=nn.BatchNorm2d, use_dropout=False, n_blocks=6, padding_type='reflect', no_antialias=False, no_antialias_up=False, opt=None):
        ...
        """
        assert(n_blocks >= 0)
        super(ResnetGeneratorNoAudio, self).__init__()
        self.opt = opt
        if type(norm_layer) == functools.partial:
            use_bias = norm_layer.func == nn.InstanceNorm2d
        else:
...
class AudioModel(nn.Module):
    ...
    def forward(self, audio):
        ...
        # a_f: [B, 256, 64, 64]
        a_f = a_f[:, :, None, None].repeat(1, 1, 64, 64)


        return spec.unsqueeze(1).repeat(1,3,1,1), a_f
