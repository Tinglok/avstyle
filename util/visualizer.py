...
def save_images(webpage, visuals, image_path, aspect_ratio=1.0, sr=16000, width=256):
    ...
    name = os.path.splitext(short_path)[0].replace('#','_').replace('!!','')
    if '30_Minutes' in name:
        name = name.split('30_Minutes')[1]
    ...
    for label, im_data in visuals.items():
        if not 'wav' in label:
            ...
            audio_name = '%s/%s.wav' % (label, name)
            os.makedirs(os.path.join(image_dir, label), exist_ok=True)
            save_path = os.path.join(image_dir, audio_name)
            sf.write(save_path, im_data[0].cpu().numpy(), samplerate=sr)
            ims.append(audio_name)
            txts.append(label)
            links.append(audio_name)
